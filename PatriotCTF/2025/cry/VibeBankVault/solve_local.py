...
def check_candidate(args):
    """
    Fungsi ini akan dijalankan secara paralel.
    Menerima tuple: (candidate_string, target_hash)
    """
    candidate, target_hash = args
    
    # Logic hashing (dicopy biar worker mandiri)
...
def solve_level_1_parallel(io):
    ...
    # 1. Generate semua kemungkinan (3844 list)
    # Kita pakai generator biar hemat memory
    candidates = []
    ...
    # 2. Eksekusi Paralel
    # ProcessPoolExecutor akan menyebar tugas ke semua core CPU
    with concurrent.futures.ProcessPoolExecutor() as executor:
        # Map fungsi check_candidate ke list candidates
        # Kita pakai chunksize biar overhead inter-process ga bikin lambat
